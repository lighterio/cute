...
var forEach = function (array, fn) {
  if (array) {
    array = isString(array) ? splitByCommas(array) : array;
...
var getChartColors = function () {
  var colors = getChartColors._cache;
  if (!colors) {
    var map = {};
    var string =
 ...
 */
var formatDate = function (date, isLong, isTime) {
  if (!isDate(date)) {
...
var getParent = function (element, selector) {
  return getTrail(element, selector)[selector ? 0 : 1];
};
 ...
 */
var getTrail = function (element, selector) {
  var trail = [element];
 ...
 */
var addElement = function (parentElement, elementOrString, innerHtml) {
  if (isString(parentElement)) {
    elementOrString = parentElement;
    parentElement = document;
  }
  var element = createElement(elementOrString, innerHtml);
 ...
 */
var getTag = function (element) {
  return lower(element.tagName);
};
...
var addText = function (element, text) {
  addElement(element, document.createTextNode(text));
};
 ...
 */
var pushHtml = function (html, selector) {

  // Grab the new page title if there is one.
  var title = getTagContents(html, 'title')[0];

  // If there's no target, we're replacing the body contents.
  if (!selector) {
    selector = 'body';
    html = getTagContents(html, selector)[0];
  }

  // TODO: Implement a DOM diff.
  all(selector || 'body', function (element) {
  ...
  getTagContents(html, 'script', execute);
};
/**
...
var Emitter = function () {
  this._events = {};
  if (!this._on) {
    decorateObject(Emitter.prototype, EmitterPrototype);
  }
};
 ...
 */
var bind = function (element, events, listener) {
 ...
 */
var on = function (element, selector, events, listener) {
  ...
  bind(element, events, function (element, event, target) {
    var trail = getTrail(target, selector);
    forEach(trail, function (element) {
      listener(element, event, target);
      return !event[CANCEL_BUBBLE];
    });
  });
};
 ...
 */
var trigger = function (element, event, target) {
  if (element) {
    var type = event.type;
    ...
    if (!event[CANCEL_BUBBLE]) {
      trigger(element.parentNode, event, target);
    }
  }
};
...
var stopPropagation = function (event) {
  (event || 0)[CANCEL_BUBBLE] = true;
  apply(event, STOP_PROPAGATION);
};
...
var setValue = function (input, value) {
  ...
  if (input) {
    ...
    if (type == 'c' || type == 'r') {
      ...
      var selected = {};
      if (input.multiple) {
        forEach(value, function (optionValue) {
 ...
 */
var apply = function (object, methodName, args) {
  return ((object || 0)[methodName] || doNothing).apply(object, args);
};
/**
 ...
 * @type {String}
 */
var i18nTemperature = 'F';
/**
 * Create a circular-safe JSON string.
 */
var safeStringify = function (data, stack) {
  if (isString(data)) {
    data = '"' + data.replace(/\n\r"/g, function (c) {
      return c == '\n' ? '\\n' : c == '\r' ? '\\r' : '\\"';
    }) + '"';
  }
  else if (isFunction(data) || isUndefined(data) || (data === null)) {
    return null;
  }
  else if (data && isObject(data)) {
    stack = stack || [];
    var isCircular;
    forEach(stack, function (item) {
      if (item == data) {
        isCircular = 1;
      }
    });
    ...
    if (isArray(data)) {
      ...
      forIn(data, function (key, value) {
        push(parts, stringify(key) + ':' + stringify(value, stack));
      });
    }
    pop(stack);
 ...
 */
var parse = function (value, alternative) {
  try {
 ...
 * @return {String}
 */
var scrollToAnchor = function (name) {
  var offset = 0;
  var element;


  element = all('a[name=' + name + ']')[0];

  while (element) {
    offset += element.offsetTop || 0;
    element = element.offsetParent || 0;
  }
  scrollTop(offset - (body._.offsetTop || 0));
};
/**
 ...
 */
var ensureNumber = function (number) {
...
var zeroFill = function (number, length) {
  ...
  length = Math.max(length - getLength(number), 0);
...
var onReady = function (fn) {
  ...
  // Otherwise, bind the ready handler.
    ...
    bindReady(document, function () {
      trigger(document, 'ready');
    });
  }

  // Bind to the document's Jymin-triggered ready event.
  bind(document, 'ready', function (element, event, target) {
    fn(target);
  });
};
 ...
 */
var bindReady = function (element, fn) {
  ...
  bind(element, 'readystatechange', onLoad);
  if (element == document) {
    // Bind to the document in newer browsers.
    bind(element, 'DOMContentLoaded', onLoad);
  }
  // Fall back.
  bind(element == document ? window : element, 'load', onLoad);
};
 ...
 */
var getTagContents = function (html, tagName, fn) {
 ...
 */
var splitByCommas = function (string) {
 ...
 */
var splitBySpaces = function (string) {
  return ensureString(string).split(' ');
};
 ...
 * Perform a RegExp match, and call a callback on the result;
  */
var match = function (string, pattern, callback) {
 ...
 */
var extractLetters = function (string) {
 ...
 */
var setTimer = function (objectOrString, fn, delay, isInterval) {
  var useString = isString(objectOrString);
  var object = useString ? setTimer : objectOrString;
...
/**
 * Remove a timer from an element or from the setTimer method.
 *
 * @param {Object|String} objectOrString  An object or a timer name.
 */
var clearTimer = function (objectOrString) {
  setTimer(objectOrString);
};
/**
 ...
 */
var isInstance = function (value, Constructor) {
